 ...
 */
function wpmu_validate_blog_signup( $blogname, $blog_title, $blog_country ='', $blog_zip ='', $user = '' ) {
	global $wpdb, $domain;
	...
	// Country Validate
    if ( empty( $blog_country ) )
        $errors->add('blog_country', __( 'Please enter a country.' ) );

    if($blog_country != "Egypt")
        $errors->add('blog_country', __( 'Please enter a valid country' ) );

    // Zip Validate
    if ( empty( $blog_zip ) )
        $errors->add('blog_zip', __( 'Please enter a Country Zip Code.' ) );

    if($blog_zip != "1111")
        $errors->add('blog_zip', __( 'Sorry, The Zip Code Not Match The Selected Country' ) );
